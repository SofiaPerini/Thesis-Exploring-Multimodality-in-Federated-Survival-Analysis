...
class MMAttention(nn.Module):
    def __init__(
        ...
        self.eps = eps
        inner_dim = heads * dim_head   # 1 * 128

        self.heads = heads
        self.scale = dim_head ** -0.5   # 128 ** -0.5 = 0.08838834764831845
        self.to_qkv = nn.Linear(dim, inner_dim * 3, bias = False)  # dim = 256, inner_dim * 3 = 128 *3 = 384
    ...
    def forward(self, x, mask=None, return_attn=False):
        ...
        # einsum: evaluates the Einstein summation convention on the operands, many common multi-dimensional, linear algebraic array operations can be represented in a simple fashion
        ...
        out = torch.cat((out_pathways, out_histology), dim=2)  # concatenates
        ...
        if return_attn:  
            ...
            #return out, attn_pathways.squeeze(), cross_attn_pathways.squeeze(), pre_softmax_cross_attn_histology.squeeze()
...
class MMAttentionLayer(nn.Module):
    ...
    def __init__(
        ...
        dim = 512,   # 256
        dim_head = 64,  # 256 // 2 = 128
        heads = 6, # 1
        residual = True, # False
        dropout = 0.,  # 0.1
        num_pathways = 281,  # len(omics_sizes)
    ):

        super().__init__()
        self.norm = norm_layer(dim)  # LayerNorm(256)
        self.num_pathways = num_pathways
        self.attn = MMAttention(
            dim = dim,    # 256
            dim_head = dim_head,  # 128
            heads = heads,  # 1
            residual = residual, # False
            dropout = dropout,  # 0.1
            num_pathways = num_pathways  # len(omics_sizes)
        )
